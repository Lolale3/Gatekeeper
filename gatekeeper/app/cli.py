...
from ..data.freight import SyntheticFreightLoader, freight_schema
...
def _load_inputs(path, loader):
    if path is None:                                  # default: gate a few dataset examples
        return [ex.to_record() for ex in loader.load()[:4]]
    records = []
...
def main(argv=None) -> int:
    ...
    p.add_argument("--freight", action="store_true", help="use the synthetic freight-email dataset + schema")
    ...
    if args.freight:
        schema, loader = freight_schema(), SyntheticFreightLoader()
    else:
        schema, loader = invoice_schema(), FixtureInvoiceLoader()
    ...
    if args.load_calibrator:
        ...
        cal_split = split_examples(loader.load()).calibration
    ...
    results = []
    for rec in _load_inputs(args.input, loader):
